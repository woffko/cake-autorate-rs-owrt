...
EOF
	printf '%s\n' '{"state":"failed","schema_version":3,"runtime_restored":false,"recovery_pending":true}' > "$work/error.pending.json"
	: > "$work/heartbeat"
}
...
CAKE_AUTORATE_RECOVER_SOURCE_ONLY=1 sh -c '
	...
	printf "%s\n%s\n" \
		"{\"state\":\"failed\",\"schema_version\":3,\"runtime_restored\":false,\"recovery_pending\":true}" \
